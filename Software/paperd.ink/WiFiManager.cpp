...
uint8_t WiFiManager::waitForConnectResult() {
  if (_connectTimeout == 0) {
    ...
    DEBUG_WM (F("Waiting for connection result with time out of"));
    Serial.printf("%d\n",_connectTimeout);
...
void WiFiManager::handleRoot() {
  ...
  if(_userconnected != NULL && _userconnectedflag == false){
    Serial.println("[RGDEBUG] Calling _userconnected callback");
    _userconnected();
    _userconnectedflag = true; // call callback only once.  
  }
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  String page = FPSTR(HTTP_HEADER);
  page.replace("{v}", "Configuration Page");
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_HEAD_END);
...
void WiFiManager::setUserConnectedCallback( void (*func)(void) ) {
  _userconnected = func;
}
